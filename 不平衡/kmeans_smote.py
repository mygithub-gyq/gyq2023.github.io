...
class KMeansSMOTE(BaseOverSampler):
    ...
    def _validate_smote_args(self, smote_args, minority_count):
        ...
        # Remove n_jobs if it's in smote_args (as SMOTE does not accept this parameter)
        if 'n_jobs' in smote_args:
            del smote_args['n_jobs']
    ...
    def _set_subalgorithm_params(self):
        ...
        # copy n_jobs to sub-algorithms (only to KMeans, not SMOTE)
        if self.n_jobs is not None:
            if 'n_jobs' not in self.smote_args:
                # Remove n_jobs from smote_args if it's present
                self.smote_args.pop('n_jobs', None)
